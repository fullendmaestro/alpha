    ...
    if (hasProperties) {
      ...
      for (const [key, value] of Object.entries(schema.properties)) {
        if (value && typeof value === "object") {
          // Recursively sanitize, increasing depth
          cleaned.properties[key] = sanitizeSchemaForGoogle(value, depth + 1);
        }
      }
    }
      ...
      mcpTools
        ...
        .map(async (tool: MCPTool) => {
          try {
            ...
            // Sanitize schema for Google LLM compatibility
            const sanitizedSchema = sanitizeSchemaForGoogle(tool.inputSchema);
